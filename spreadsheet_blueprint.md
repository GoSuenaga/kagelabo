...
4. **全カットに必ず動きの指示を入れる** — 食事俯瞰・風景など静止しがちなシーンでも「手が伸びてくる」「カメラがゆっくりパン」等の動きを明示する。完全に静止した映像はNG（絶対原則: Micro-movements）
5. **ロゴ表示カットの動画プロンプトにロゴ指示を入れない** — ロゴはCreatomateで後から重ねる。動画プロンプトに「Logo」と書くとAIが背景にロゴを生成してしまう。代わりに「No text, no logos, no signage」を入れる
6. **手指のクローズアップは避ける** — AI動画生成では手指の描写が高確率で破綻する（指の本数が増える、関節がおかしい等）。手を映す場合は引きのショットで小さく映すか、物で手が隠れる構図にする。代替案：肩越しショット、後ろ姿、足元、物だけのショット
...
4. 全カットに必ず動きの指示を入れる — 静止しがちなシーンでも手の動き・カメラパン等を明示
5. ロゴ表示カットの動画プロンプトにロゴ指示を入れない — 「No text, no logos, no signage」を入れる
6. 手指のクローズアップは避ける — 指の本数が増える等の破綻が多い。引きのショットか物で隠す構図にする
